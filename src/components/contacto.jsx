...
export default function Contacto({ contactos }) {
  return (
    <section className="flex flex-col w-full gap-2">
      ...
      <section className="grid w-full grid-cols-1 gap-4 my-8 md:grid-cols-3">
        {contactos.map(({ nombre, url, user, icono }) => (
          <a key={nombre} href={url} target="_blank" rel="noreferrer">
            <Card className="flex flex-col w-full h-full p-4 transition-colors bg-black border border-white bg-gradient-to-tl from-black/80 via-zinc-600/20 to-black/80 hover:bg-zinc-950 hover:border-white md:border-zinc-800">
